...
public class MultaListAdapter extends BaseAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ViewHandler h = null;
        if(view==null){
            view = layoutInflater.inflate(R.layout.multa_item_layout,parent);
    ...
    private void fillView(ViewHandler h, int position){
        ...
        h.vMotivo.setText(multas.get(position).motivo);
        h.vStatus.setText(multas.get(position).status);

        if(multas.get(position).status.equals(Multix.PAGADA)){
            h.vStatus.setTextColor(Color.GREEN);
        }else{
